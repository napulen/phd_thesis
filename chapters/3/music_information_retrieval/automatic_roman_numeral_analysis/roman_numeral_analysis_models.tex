...
and key estimation \parencite{temperley1999whats}
approaches. The collective algorithms were implemented in
...
representations of both inputs and outputs. Sapp filled in
the remaining gap by providing additional stages that
allowed \emph{Melisma} to digest Humdrum(\texttt{**kern})
representations and output annotated Humdrum(\texttt{**harm}
\gls{rna} annotations generated automatically. These
additional stages were summarized by a program called
\emph{tsroot} \parencite{sapp2009tsroot} and used to
generate fully automatic \gls{rna} of the \emph{KernScores}
database \cite{sapp2005online}.
