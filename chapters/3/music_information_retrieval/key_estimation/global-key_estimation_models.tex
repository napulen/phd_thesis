...
% corpus of 8325 Finnish folk songs in \gls{midi} format, the
...
% corpus of 410 classical music \gls{midi} files, where only the
...
% \gls{midi} key-finding model operated directly on the \gls{midi}
% files. The maximum key identification rates for the peak
% detection, fuzzy analysis, and \gls{midi} key-finding models
% were 70.17\%, 75.25\%, and 80.34\%, respectively.

% we evaluate the technique by comparing the results of
% symbolic (\gls{midi}) key finding, audio key finding with peak
...
first \emph{Audio Key-Finding} \gls{mirex} evaluation, in 2005.
...
the Key and Chord detection tasks from \gls{mirex}. For testing
...
of several \gls{midi} files. These harmonic analyses were
...
consisted 1046 \gls{midi} files of Rock music. The annotated files
...
and an audio-synthesized dataset of \gls{midi} files of classical
...
music. The model was submitted to the \gls{mirex} campaign of 2018
...
little agreement (except for the \gls{mirex} campaigns) in the
