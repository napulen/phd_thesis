...
but assumed that any arbitrary 20-second fragment of the
audio input will present the global key, disallowing the
model to operate with music that features modulations. The
model was trained on three datasets: the GiantSteps
\gls{mtg} Key dataset, the McGill Billboard dataset, and an
internal dataset with classical music. The model was
