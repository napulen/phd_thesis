...
$\text{pcset}(\elkey$:$\elnum) = \elpcset$. Thus,
$\Gamma(\pcsetcega, \keyC) = \rnvisev$, $\Gamma(\pcsetcega,
\keye) = \rnivsev$, and so on.

\begin{equation}
    \label{eq:numerator_from_pcset_key}
    \begin{split}
        \elpcset = \pcsetcega & \elkey = \keyC & 
        \Gamma(\elpcset, \elkey) = \rnvisev \\
        \elpcset = \pcsetcega & \elkey = \keye & 
        \Gamma(\elpcset, \elkey) = \rnivsev \\
        \text{and so on}
    \end{split}
\end{equation}
