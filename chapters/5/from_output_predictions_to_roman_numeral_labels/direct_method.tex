...
required to generate the \gls{rna} label $\elrna$, only the
inversion requires some additional computations. The rest
are direct predictions from the \gls{crnn}. One issue that needs to be addressed before retrieving the predictions, however, is the chord segmentation. 
