...
a \gls{pcset} $\elpcset$ and key $\elkey$ using a custom
algorithm called Numerator and Tonicization Estimation
Method (\gls{natem}). This algorithm determines which Roman
numeral numerator to assign given the key. In instances
where no Roman numeral numerator is suitable for the
\gls{pcset} and key provided, the algorithm searches for an
appropriate tonicization and returns a Roman numeral label
with a tonicized key. The algorithm is described in detail
