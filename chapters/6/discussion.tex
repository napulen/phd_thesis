...
is the only model that seems to recognize the chord
discussed above, the $\rnGer$ \gls{augsix} in measure 8. The
$\rnIt$ in \textcite{chen2021attend} is expected, because
this model collapses all flavours of \gls{augsix} chords
into one category.\footnote{This can also be confirmed in
the confusion matrix of
\reffig{confusion_matrix_chensu2021}, where $\rnGer$ and
$\rnFr$ instances are often predicted as $\rnIt$.} which
makes it difficult (maybe impossible) to distinguish between
a $\rnIt$ and $\rnGer$. Notice also that there are two
