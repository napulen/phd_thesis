...
% A less prominent change is the one related the use of a
% constant number of filters. Although the performance is
% lower than the baseline, it lies within a standard
% deviation for all tasks, a detrimental effect that seemed
% more impactful during preliminary experiments.
Most modifications in the ablation studies have a negative
effect compared to the baseline model, which is consistent
with the observations during preliminary experiments
designing the network. 
% One exception here is linear dense
% layer, which seems to provide a slightly better performance
% than the baseline. 
Thus, the baseline model presented in
this section was used in all subsequent experiments.
% This was an unexpected result, because that did not seem
% to be the case in preliminary experiments. It could be
% that the nonlinear activation in the dense layers of the
% baseline model work better when data augmentation is
% introduced.
